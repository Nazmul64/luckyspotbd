...
<div class="row">
    <div class="col-12 mx-auto">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('supportlink.update',$supports_link->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                     @method('PUT')
                    <div class="mb-3">
                        <label class="form-label">Support Link Name</label>
                        <input type="url" name="support_link" class="form-control"
                               value="{{ old('support_link',$supports_link->support_link ?? '') }}" placeholder="Enter bank name">
                     ...
                     <div class="mb-3">
                        ...
                        <input type="text" name="title" class="form-control"
                               value="{{ old('title',$supports_link->title ?? '') }}" placeholder="Enter bank name">
                     ...
                     <div class="mb-3">
                        ...
                        @error('new_photo') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <img id="photoPreview"
                             src="{{ $supports_link->photo ? asset('uploads/supports/' . $supports_link->photo) : '#' }}"
                             alt="Preview"
                             style="max-width: 150px; {{ $supports_link->photo ? '' : 'display:none;' }} border: 1px solid #ccc; padding: 5px; border-radius: 5px;">
                    </div>
